...
El área de BigDataAnalytics de BMV nace para esta misión, combinando 3 disciplinas que hasta hace poco no se relacionaban: el dominio de negocio para hacer las preguntas correctas, la estadística avanzada para describir el problema, las ciencias computacionales para prescribir soluciones y predecir resultados. Como muestra de nuestra labor, abajo presentamos un análisis descriptivo de una muestra de colaboradores del grupo tomada en Diciembre 2014.
...
meantimeperlocation <- summarise(group_by(empentryroll, Delegacion...Municipio), tiempo_promedio=mean(horaEntrada), personal=n())
meantimeperlocation <- meantimeperlocation[order(meantimeperlocation$tiempo_promedio), ]
...
#plot <- nPlot(personal ~ tiempo_promedio, data = meantimeperlocation, type = 'scatterChart')
#plot$xAxis(axisLabel = 'Hora entrada promedio')
#plot$chart(tooltipContent = "#! function(key, x, y, e){return '<b>' + e.point.DelegMun + '</b>'} !#")
#plot$show('inline', include_assets = TRUE)
rPlot(personal ~ tiempo_promedio, data = meantimeperlocation, type = 'point', tooltip = "#!function(item){ return '<b>' + item.DelegMun + '</b>' }!#")  
```